// js/menu.js
import { getCurrentCategory, setItems } from './state.js';

let currentCategory = getCurrentCategory();

export function renderMenuItems(items, fallbackCategory = 'all') {
  ...
  items.forEach((item, index) => {
    ...
    const categoryClass = item.category || fallbackCategory;
      ...
      optionsHtml = `
        <div class="options" style="display:flex;flex-direction:column;gap:8px;margin-top:12px;">
          ${item.variants.map((v, vIndex) => {
            ...
            const casePrice = casePriceNum ? casePriceNum.toFixed(2) : "";
      ...
      <div class="col-sm-6 col-lg-4 all ${categoryClass} menu-item"
        ...
        <div class="box">
          ...
          <div class="detail-box">
            ${brand ? `<span class="brand-pill" data-brand="${brand}">${brand}</span>` : ""}
            <h5>${item.name}</h5>
    ...
    if (added) setTimeout(() => added.classList.add('show'), 40 * index);
  });
}
...
export async function loadMenu(category = currentCategory) {
  ...
  try {
    ...
    // por si algunos no traen category, se la ponemos
    const normalized = items.map((it) => ({ ...it, category: it.category || category }));

    renderMenuItems(normalized, category);
  } catch (err) {
    console.error(`Error cargando la categoría ${category}:`, err);
  }
}

export async function preloadAllMenus(categories) {
  try {
    const results = await Promise.all(
      categories.map(async (cat) => {
        ...
        return items.map((it) => ({ ...it, category: it.category || cat }));
      })
    );
    ...
    console.error("Error precargando menús:", e);
    setItems([]);
    return [];
  }
}
